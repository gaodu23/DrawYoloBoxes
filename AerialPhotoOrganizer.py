...
def parse_kml_nested(kml_path):
    """
    使用嵌套文件夹模式解析KML文件，通过Folder嵌套关系确定级别
    ...
    try:
        ...
        # 处理根文件夹下的县市级文件夹
        for district_folder in root.findall('.//kml:Folder', KML_NS):
            # 尝试查找name标签，如果没有则尝试查找n标签
            district_name_elem = district_folder.find('./kml:name', KML_NS)
            if district_name_elem is None:
                district_name_elem = district_folder.find('./kml:n', KML_NS)
                
            if district_name_elem is None or not district_name_elem.text:
                continue
                
            district_name = district_name_elem.text.strip()
            district_region = AdministrativeRegion(district_name, 3)  # 县市级
            
            # 解析县市级边界
            parse_boundary_for_region(district_folder, district_region, KML_NS)
            
            if district_region.polygon or len(regions_by_level[3]) == 0:
                regions_by_level[3].append(district_region)
            
            # 处理乡镇级文件夹
            town_folders = district_folder.findall('./kml:Folder', KML_NS) or district_folder.findall('./Folder')
            for town_folder in town_folders:
                # 尝试查找name标签，如果没有则尝试查找n标签
                town_name_elem = town_folder.find('./kml:name', KML_NS)
                if town_name_elem is None:
                    town_name_elem = town_folder.find('./kml:n', KML_NS)
                    
                if town_name_elem is None or not town_name_elem.text:
                    continue
                    
                town_name = town_name_elem.text.strip()
                town_region = AdministrativeRegion(town_name, 2)  # 乡镇级
                
                # 解析乡镇级边界
                parse_boundary_for_region(town_folder, town_region, KML_NS)
                
                if town_region.polygon or len(town_folder.findall('./kml:Folder', KML_NS) or town_folder.findall('./Folder')) > 0 or len(town_folder.findall('./kml:Placemark', KML_NS) or town_folder.findall('./Placemark')) > 0:
                    regions_by_level[2].append(town_region)
                    # 正确设置乡镇与县市的关系
                    town_region.parent = district_region  # 乡镇的父级是县市
                    if town_region not in district_region.children:
                        district_region.children.append(town_region)
                
                # 处理村庄级文件夹
                village_folders = town_folder.findall('./kml:Folder', KML_NS) or town_folder.findall('./Folder')
                for village_folder in village_folders:
                    # 尝试查找name标签，如果没有则尝试查找n标签
                    village_name_elem = village_folder.find('./kml:name', KML_NS)
                    if village_name_elem is None:
                        village_name_elem = village_folder.find('./kml:n', KML_NS)
                        
                    if village_name_elem is None or not village_name_elem.text:
                        continue
                        
                    village_name = village_name_elem.text.strip()
                    village_region = AdministrativeRegion(village_name, 1)  # 村庄级
                    village_region.parent = town_region  # 设置村庄的父级为乡镇
                    
                    # 解析村庄级边界
                    parse_boundary_for_region(village_folder, village_region, KML_NS)
                    
                    if village_region.polygon:
                        regions_by_level[1].append(village_region)
                        if village_region not in town_region.children:
                            town_region.children.append(village_region)
                
                # 处理乡镇下直接的地标点（作为村庄）
                placemarks = town_folder.findall('./kml:Placemark', KML_NS) or town_folder.findall('./Placemark')
                for placemark in placemarks:
                    # 尝试查找name标签，如果没有则尝试查找n标签
                    placemark_name_elem = placemark.find('./kml:name', KML_NS)
                    if placemark_name_elem is None:
                        placemark_name_elem = placemark.find('./kml:n', KML_NS)
                        
                    if placemark_name_elem is None or not placemark_name_elem.text:
                        continue
                        
                    placemark_name = placemark_name_elem.text.strip()
                    village_region = AdministrativeRegion(placemark_name, 1)  # 村庄级
                    village_region.parent = town_region  # 设置村庄的父级为乡镇
                    
                    # 解析边界
                    coords_elem = placemark.find('.//kml:coordinates', KML_NS) or placemark.find('.//coordinates')
                    if coords_elem is not None and coords_elem.text:
                        coords = parse_coordinates(coords_elem.text)
                        if len(coords) >= 3:
                            village_region.polygon = Polygon(coords)
                    
                    # 无论是否有边界，都添加到regions_by_level[1]
                    regions_by_level[1].append(village_region)
                    # 添加到乡镇的子区域
                    if village_region not in town_region.children:
                        town_region.children.append(village_region)
            ...
            # 处理县市下直接的地标点（作为乡镇）
            placemarks = district_folder.findall('./kml:Placemark', KML_NS) or district_folder.findall('./Placemark')
            for placemark in placemarks:
                # 尝试查找name标签，如果没有则尝试查找n标签
                placemark_name_elem = placemark.find('./kml:name', KML_NS)
                if placemark_name_elem is None:
                ...
                placemark_name = placemark_name_elem.text.strip()
                town_region = AdministrativeRegion(placemark_name, 2)  # 乡镇级
                town_region.parent = district_region  # 设置乡镇的父级为县市
                ...
                if coords_elem is not None and coords_elem.text:
                    coords = parse_coordinates(coords_elem.text)
                    if len(coords) >= 3:
                        town_region.polygon = Polygon(coords)
                
                # 无论是否有边界，都添加到regions_by_level[2]
                regions_by_level[2].append(town_region)
                # 添加到县的子区域
                if town_region not in district_region.children:
                    district_region.children.append(town_region)
...
def find_region_for_point(point, regions_by_level):
    ...
    Returns:
        district, town, village: 县市、乡镇、村庄三级行政区划
    """
    district = None
    town = None
    village = None
    ...
    for d in regions_by_level[3]:
        if d.polygon and d.polygon.contains(point):
            district = d
            break
    
    # 再找到包含该点的乡镇
    for t in regions_by_level[2]:
        if t.polygon and t.polygon.contains(point):
            town = t
            break
    
    # 最后找到包含该点的村庄
    for v in regions_by_level[1]:
        if v.polygon and v.polygon.contains(point):
            village = v
            break
